...
export const positiveNumber = z.number().min(0).default(1);
...
export const initialFilter: Decal = {
  scale: 1,
...
export function useFilter() {
  const [filter, setFilter] = useQueryStates(
    {
      decal: parseAsJson(decalSchema).withDefault(initialFilter),
    },
    { clearOnDefault: false }
  );

  const serialize = createSerializer(decalSchema);
  const queryString = serialize({ decal: filter.decal });
